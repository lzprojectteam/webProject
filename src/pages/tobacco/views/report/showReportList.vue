  ...
  watch: {
    $route(to, from) {
      if (
        ...
        to.meta.file.indexOf("showReportList") !== -1
      ) {
        this.initComponent();
      }
    }
  ...
  methods: {
    ...
    handleScroll() {
      ...
      if (this.scrollCount > 5) {
        ...
        let table1_width = document.querySelector(".dataTable").offsetWidth;
        ...
        let tableHeads = document.querySelectorAll(".dataTable .r2"); //获取表头
        if (tableHeads === null) {
        ...
        if (this.firstLoad) {
          ...
          let colgroup = document.querySelector(".dataTable .t1 colgroup"); //获取设置表格宽度的那部分html
