  ...
  <div class='mainPanel'>
    ...
    <main class='contentPanel'>
      <el-table
        ...
        <el-table-column prop="license" :label="this.$t('tobacco.tstorage.vehicle.license')" min-width="100"
                         show-overflow-tooltip/>
        <el-table-column prop="driver" :label="this.$t('tobacco.tstorage.vehicle.driver')" min-width="100"
                         show-overflow-tooltip/>
        <el-table-column prop="legalPerson" :label="this.$t('tobacco.tstorage.vehicle.legalPerson')" min-width="100"
                         show-overflow-tooltip/>
        <el-table-column prop="mobile" :label="this.$t('tobacco.tstorage.vehicle.mobile')" min-width="100"
                         show-overflow-tooltip/>
        <el-table-column prop="displacement" :label="this.$t('tobacco.tstorage.vehicle.displacement')" min-width="100"
                         show-overflow-tooltip/>
        <el-table-column prop="capacity" :label="this.$t('tobacco.tstorage.vehicle.capacity')" min-width="100"
                         show-overflow-tooltip/>
        <el-table-column prop="listingDate" :label="this.$t('tobacco.tstorage.vehicle.listingDate')" min-width="100"
                         show-overflow-tooltip>
          <template slot-scope="scope">
            {{dateFormat(scope.row.listingDate,'yyyy-mm-dd')}}
          </template>
        </el-table-column>
        <el-table-column
          fixed="right"
...
<script>
  const AddForm = () => import('./vehicleAdd.vue');
  ...
  export default {
    ...
    methods: {
      ...
      deleteButtonClick() {
        ...
        Promise.all([vehicleApi.deleteVehicle(this.formData.selectRow.id)])
          .then(([response]) => {
            this.$message({
              type: 'info',
      ...
      onSearchButtonClick() {
        Promise.all([vehicleApi.getAll({
          ...
          contains: 'license,driver,legalPerson,displacement,capacity:{keyword}:true'.format({keyword: this.formData.pagination.keyword}),
          search: 'organization.organizationId:rlike:{orgid};'.format({
            orgid: this.searchData.organizationId,
          })
        })])
          .then(([response]) => {
            this.formData.vehicleList = response.content;
