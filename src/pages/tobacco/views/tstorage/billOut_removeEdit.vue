    ...
    <el-row>
      ...
      <el-col :span="8">
        <el-form-item :label="$t('tobacco.tstorage.billOut.organizationName')" prop="destOrganizationId">
          <el-select v-model="formItem.destOrganizationId" @change="SourceOrganizationChange" placeholder="请选择"
                     style="width: 100%;">
            <el-option
              v-for="item in formData.sourceOrganizationList"
      ...
      <el-col :span="8">
        <el-form-item :label="this.$t('tobacco.tstorage.region.whouseId')" prop="warehouse">
          <el-select v-model="formItem.warehouse2" clearable @change="getRegion2(true)" style="width: 100%;">
      ...
      <el-col :span="8">
        <el-form-item :label="$t('tobacco.tstorage.facility.regionId')" prop="region">
          <el-select v-model="formItem.region2" clearable style="width: 100%;">
    ...
    methods: {
      ...
      load() {
        ...
        Promise.all([billInApi.getAll({
          ...
          .then(([response]) => {
            this.$set(this.formItem, 'warehouse2', response.content[0]['warehouse']['id']);
            this.$set(this.formItem, 'region2', response.content[0]['region']['id']);
      ...
      getSourceOrangnization(flag) {
        ...
        this.formData.regionList2 = [];
        //获取三方组织及其库房数据
        Promise.all([billOutApi.getWarehouseList({
