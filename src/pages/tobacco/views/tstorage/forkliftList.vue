  ...
  <div class='mainPanel'>
    ...
    <main class='contentPanel'>
      <el-table
        ...
        <el-table-column prop="number" :label="this.$t('tobacco.tstorage.forklift.number')" min-width="100"
                         show-overflow-tooltip
        />
        <el-table-column prop="title" :label="this.$t('tobacco.tstorage.forklift.title')" min-width="100"
                         show-overflow-tooltip
        />
        <el-table-column prop="author" :label="this.$t('tobacco.tstorage.forklift.author')" min-width="100"
                         show-overflow-tooltip
        />
        <el-table-column prop="date" :label="this.$t('tobacco.tstorage.forklift.date')" min-width="100"
                         show-overflow-tooltip
        >
        ...
        <el-table-column prop="available" :label="this.$t('tobacco.tstorage.forklift.available')" min-width="100"
                         show-overflow-tooltip
        >
          <template slot-scope="scope">
        ...
        <el-table-column prop="desc" :label="this.$t('tobacco.tstorage.forklift.desc')" min-width="100"
                         show-overflow-tooltip
        />
        <el-table-column
          fixed="right"
...
<script>
  const AddForm = () => import('./forkliftAdd.vue');
  const EditForm = () => import('./forkliftEdit.vue');
  ...
  export default {
    ...
    methods: {
      ...
      deleteButtonClick() {
        ...
        Promise.all([forkliftApi.deleteForklift(this.formData.selectRow.id)])
          .then(([response]) => {
            this.$message({
              type: 'info',
      ...
      onSearchButtonClick() {
        Promise.all([forkliftApi.getAll({
          ...
          sort: 'number,ASC',
          contains: 'number,title,author,date,available,desc,:{keyword}:true'.format({keyword: this.formData.pagination.keyword}),
          search: 'organization.organizationId:rlike:{orgid};number:EQ:{number};available:EQ:{available};desc:EQ:{desc};'.format({
            number: this.searchData.number,
            title: this.searchData.title,
            author: this.searchData.author,
            date: this.searchData.date,
            available: this.searchData.available,
